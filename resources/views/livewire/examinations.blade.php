<div style="position: relative" x-data="{ 
        ...
        <div class="col-md">
            @forelse ($regulations as $r)
                ...
                <div class="form-floating">
                    <select class="form-select" id="regulation" wire:model="regulation_id">
                @endif
            ...
            @foreach ($c->subjects ? $c->subjects : [] as $s)
                <tr>
                    <td><input class="form-check-input" type="checkbox" 
                        value="{{ $exam_schedule[$s->subject_code]['selected'] }}" 
                        wire:model="exam_schedule.{{ $s->subject_code }}.selected">
                    </td>
                    ...
                    <td><input class="form-control" 
                        x-bind:disabled="!examSchedule['{{ $s->subject_code }}']['selected']" 
                        x-bind:class="examSchedule['{{ $s->subject_code }}']['state'] == false ? 'form-control border-danger' : 'form-control' "
                        type="date" name="exam-date" id="exam-date" 
                        value="{{ $exam_schedule[$s->subject_code]['exam_date'] }}" 
                        wire:model="exam_schedule.{{ $s->subject_code }}.exam_date">
                    </td>
    ...
    @empty
        <div class="text-muted">
            Data not found. Nothing to schedule.
        </div>
    @endforelse
        </div>
        <div class="card-footer">
            <button class="btn btn-success float-end" wire:click="store">Create Schedule</button>
        </div>
      </div>
    ...
    <!-- toast -->
    <div class="toast position-fixed top-0 end-0 m-4 bg-secondary border-0" role="alert" aria-live="assertive" aria-atomic="true">
